...
$allowed=@((Join-Path $projectRoot 'Builds\PerformanceBaseline\Windows'),(Join-Path $projectRoot 'Builds\PerformanceCandidate\Windows'))
if($buildRoot -notin $allowed){throw 'Benchmark requires an isolated baseline or candidate package.'}
$out=Join-Path $projectRoot "Saved\PerformancePass\$Label"
New-Item -ItemType Directory -Path $out -Force | Out-Null
$saved=Join-Path $buildRoot 'DungeonCrawler\Saved'
New-Item -ItemType Directory -Path "$saved\SaveGames" -Force | Out-Null
foreach($name in @('ExistingSave','Floor_00','Floor_02','Floor_16','Explored')){
 Copy-Item -LiteralPath (Join-Path $projectRoot "Saved\PerformancePass\Fixtures\$name.sav") -Destination "$saved\SaveGames\ExpansionPrototype_Auto.sav" -Force
