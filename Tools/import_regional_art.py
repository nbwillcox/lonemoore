...
M=json.loads((R/'manifest.json').read_text());ROOT='/Game/RegionalArt';T=u.AssetToolsHelpers.get_asset_tools();A=u.EditorAssetLibrary;E=u.MaterialEditingLibrary
...
if stage=='Textures':
 ...
 default=next(a for a in M['materials'] if a['role']=='metal')
 ...
 uvparent=parent('M_RegionalUV');worldparent=parent('M_RegionalWorld',True);decparent=parent('M_RegionalDecal',decal=True)
 for a in M['materials']+M['decals']:
  for prefix,par in ([('MI_',decparent)] if a['role']=='decal' else [('MI_',worldparent),('MI_UV_',uvparent)]):
   ...
   E.set_material_instance_scalar_parameter_value(mi,'Metallic',1 if 'metallic' in a['maps'] else 0);E.set_material_instance_scalar_parameter_value(mi,'EmissionStrength',1.5 if 'emission' in a['maps'] else 0);E.update_material_instance(mi);A.save_loaded_asset(mi)
else:
 for a in M['props']:
